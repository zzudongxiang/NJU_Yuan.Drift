...
from ctypes import c_int32, windll, pointer
...
class Motor:
    ...
    def SetBackground(self):
        ...
        # 读取当前位置作为背景
        for Index in range(2):
            self.MT_API.MT_Get_Axis_Software_P_Now(Index, pointer(self.iValue))
            self.Background[Index] = self.iValue.value
    ...
    def SetOffset(self, Offset):
        ...
        # 计算偏移量, 换算位脉冲给到位移台
        for Index in range(2):
            Num = self.Background[Index] + int(Offset[Index] * self.StepRate[Index])
            if Num != 0:
                self.MT_API.MT_Set_Axis_Position_P_Target_Abs(Index, Num)
